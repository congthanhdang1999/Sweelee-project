...
class DashboardController extends Controller
{
    ...
    public function show(Request $request)
    {
        ...
        $data['cancel'] = $this->order->where('status', 3)->count();
        $orders = $this->order->orderBy('id', 'DESC')->paginate(8);
        $successOrder = (int)$this->order->where('status', 2)->sum('total');

        return view('admin.dashboard', compact('orders', 'data', 'successOrder'));
    }
}
